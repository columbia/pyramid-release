...
/**
  ...
  * @param cmsDelta
  */
class UnbiasedApproximateCountTable(override val cmsEpsilon: Double,
  ...
  private val hashSeeds: Array[Int] = seeds match {
    case Some(s) => s
    case None => initializeHashSeeds(depth * 2)
  }
  ...
  protected def copyNoiseArray(): Array[Array[Double]] = {
    return noiseArray match {
      ...
      case None => Array.fill[Double](depth, width){ 0.0 }
    }
  }
  ...
  def getWidthIndex(key: Array[Double], seed: Int): Int = {
    val hash = abs(MurmurHash3.arrayHash(key, seed))
    return positiveMod(hash, width)
  }

  /**
    * Returns the sign of the key for the relevant row (given by the seed) for
    * the unbiased scheme.
    *
    * @param key
    * @param seed
    * @return +1 or -1
    */
  def getSign(key: Array[Double], seed: Int): Int = {
    val hash = abs(MurmurHash3.arrayHash(key, seed))
    return if (hash % 1 == 0) +1 else -1
  }
  ...
  override def increment(key: Array[Double], by: Double): Unit = {
    ...
    try {
      ...
      (0 until depth).foreach { depthIndex =>
        ...
        val sign       = getSign(key, hashSeeds(depthIndex + depth))
        sketchArray(depthIndex)(widthIndex) =
          sketchArray(depthIndex)(widthIndex) + sign * by
      }
    } finally {
  ...
  Double = {
    ...
    try {
      val candidates = (0 until depth).map { depthIndex =>
        val widthIndex: Int = getWidthIndex(key, hashSeeds(depthIndex))
        val sign: Int       = getSign(key, hashSeeds(depthIndex + depth))
        val noise           = getNoise(depthIndex, widthIndex, addBackingTableNoise)
        val x               = sketchArray(depthIndex)(widthIndex) + noise
        sign * x
      }.sorted
      val result = candidates(candidates.length/2)
